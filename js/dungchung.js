...
// Lưu user lên Firebase (nếu đã cấu hình firebaseDb)
function saveUserToFirebase(user) {
    try {
        if (window.firebaseDb) {
            window.firebaseDb.collection('users').doc(user.username).set(user);
        }
    } catch (e) {
        console.error('Firebase saveUserToFirebase error:', e);
    }
}
...
function signUp(form) {
    ...
    // Lưu người mới lên Firebase để admin các máy khác thấy được
