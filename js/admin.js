...
window.onload = function () {
    ...
    // Luôn cho phép load admin (bỏ chặn theo localStorage.admin)
    addTableProducts();
    addTableDonHang();
    addTableKhachHang();
    addThongKe();
...
function openTab(nameTab) {
    ...
    switch(nameTab) {
        ...
        case 'Database':
            var dbSection = document.getElementsByClassName('database')[0];
            if (dbSection) {
...
// ====================== Khách Hàng =============================
// Load danh sách user từ Firebase (nếu có), fallback về localStorage nếu lỗi
function fetchUsersForAdmin(callback) {
    if (!window.firebaseDb) {
        callback(getListUser());
        return;
    }
    ...
    window.firebaseDb.collection('users').get()
        .then(function (snapshot) {
            ...
            // Nếu Firestore chưa có dữ liệu thì fallback local
            if (!users.length) users = getListUser();
            callback(users);
        })
        .catch(function (err) {
            console.error('Firebase fetchUsersForAdmin error:', err);
            callback(getListUser());
        });
}

// Vẽ bảng Khách Hàng, ưu tiên dữ liệu từ Firebase
function addTableKhachHang() {
    var tc = document.getElementsByClassName('khachhang')[0].getElementsByClassName('table-content')[0];

    fetchUsersForAdmin(function(listUser) {
        var s = `<table class="table-outline hideImg">`;
...
function voHieuHoaNguoiDung(inp, taikhoan) {
    ...
    for(var u of listUser) {
        if(u.username == taikhoan) {
            ...
            try {
                if (window.firebaseDb) {
                    window.firebaseDb.collection('users').doc(u.username).update({ off: value });
                }
            } catch (e) {
                console.error('Firebase voHieuHoaNguoiDung error:', e);
            }
    ...
    if(window.confirm('Xác nhận xóa '+taikhoan+'? \nMọi dữ liệu về '+taikhoan+' sẽ mất! Bao gồm cả những đơn hàng của '+taikhoan)) {
        ...
        for(var i = 0; i < listuser.length; i++) {
            if(listuser[i].username == taikhoan) {
                ...
                try {
                    if (window.firebaseDb) {
                        window.firebaseDb.collection('users').doc(taikhoan).delete();
                    }
                } catch (e) {
                    console.error('Firebase xoaNguoiDung error:', e);
                }
